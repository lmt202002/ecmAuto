...
public class CheckStock {
    ...
    @Test
    public void testCheckStock() throws IOException, InterruptedException {
        /**前台微商检查指定商品库存
...
//        System.out.println(list1.get(0)+"\n"+list1.get(1));
        ...
        int stockOld=Integer.parseInt(list1.get(1));//当前待收库存
        FgStep.auditOrder(driver,domain,true);
        int stockNow=stockOld+Integer.parseInt(RWuserData.readTestData(goodSnumPath,"项目1"));//审核的商品数量

        Session.logout(driver,domain);//前台退出
        driver.close();
        Assert.assertEquals(stockNow,stockOld);//比较测试
    }
}
